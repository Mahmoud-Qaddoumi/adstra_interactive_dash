...
def update_graph(*args):
    if len(args) > 1:
        ...
        time_fig, violin_fig = make_num_time_figs(df=dff, num_cols=args[0], date_col=datetime_col,
                                                  anomaly_col=anomaly_col, cat_agg_col=args[2],date_agg_col=args[1])
...
def update_network(*args):
    if len(args) > 1:
        dff = pd.read_json(StringIO(args[-1]), orient='split')
        figs = make_network_callbacks(df=dff, left_col=args[0], right_col=args[1], color_col=args[2],anomaly_col=anomaly_col)
        return figs
